...
```bash
tools/run.sh          # 임포트 캐시를 갱신한 뒤 게임 실행 (권장)
```

`godot --path .` 로 바로 실행하면, 새 스크립트를 받은 직후에는 class_name 캐시가 낡아 autoload가 로드되지 않고
레이스가 비정상 동작할 수 있다(즉시 FINISH, 재시작 불가, 그리드 배치 이상). 그때는 한 번만
`godot --headless --path . --import` 를 돌리면 해소된다.
