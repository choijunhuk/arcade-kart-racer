...
const PADDING: float = 0.08
const MIN_ASPECT: float = 0.4
const MIN_SPAN: float = 0.000001


## Projects world X/Z points into padded normalized 2D with a minimum readable aspect.
...
## Projects one world point using the same fitted reference bounds.
...
static func _project_with_bounds(point: Vector3, bounds: Rect2, span: float) -> Vector2:
	var projected: Vector2 = Vector2(point.x, point.z)
	var fitted: Vector2 = bounds.size
	var minor: float = minf(fitted.x, fitted.y)
	if minor > MIN_SPAN and minor / span < MIN_ASPECT:
		if fitted.x < fitted.y:
			fitted.x /= MIN_ASPECT
		else:
			fitted.y /= MIN_ASPECT
	else:
		fitted = Vector2.ONE * span
	return (projected - bounds.get_center()) / fitted.max(Vector2.ONE * MIN_SPAN) * (1.0 - 2.0 * PADDING) + CENTER
