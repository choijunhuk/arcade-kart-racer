...
## NetStateCodec wraps yaw to [-PI, PI) before quantizing (spec item G):
## exactly -PI stays put, +PI wraps to -PI, and an ordinary negative mid
## value must round-trip unchanged. Regression for the seam wrap itself,
## independent of the RacingLine/follower seam handling covered elsewhere.
func test_snapshot_yaw_roundtrip_at_seam_and_negative_mid_value() -> void:
	for source_yaw: float in [-PI, PI, -1.75]:
		_kart.rotation.y = source_yaw
		var result: RaceSnapshot = RaceSnapshot.unpack(_snapshot().pack())
		assert_not_null(result)
		if result == null:
			continue
		var expected: float = wrapf(source_yaw, -PI, PI)
		assert_almost_eq(float(result.karts[0]["state"]["rotation"][1]), expected, 0.00051)
