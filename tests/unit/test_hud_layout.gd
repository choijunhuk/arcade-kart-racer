...
const LAYOUT_PATH: String = "res://ui/hud/hud_layout.gd"
...
func test_major_regions_stay_in_bounds_and_do_not_overlap_at_supported_sizes() -> void:
	...
	for viewport_size: Vector2 in [Vector2(800.0, 450.0), Vector2(1600.0, 900.0)]:
		var layout: Dictionary = layout_script.call("calculate", viewport_size)
		var regions: Dictionary = layout["regions"]
		var bounds: Rect2 = Rect2(Vector2.ZERO, viewport_size)
		for name: String in MAJOR_REGIONS:
			assert_true(bounds.encloses(regions[name]), "%s stays inside %s" % [name, viewport_size])
		for first: int in range(MAJOR_REGIONS.size()):
			for second: int in range(first + 1, MAJOR_REGIONS.size()):
				var first_name: String = MAJOR_REGIONS[first]
				var second_name: String = MAJOR_REGIONS[second]
				assert_false(
