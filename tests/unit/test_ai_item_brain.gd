...
class FakeItemSlotView extends ItemSlotView:
	...
	func _init(has_item: bool, category: ItemData.ItemCategory) -> void:
		_has_item = has_item
		_category = category
...
func test_rocket_dart_eligible_when_target_is_in_the_fire_cone() -> void:
	...
	assert_true(AIItemBrain.is_rule_eligible(ItemData.ItemCategory.PROJECTILE, use_profile, context))


func test_rocket_dart_ineligible_with_nothing_ahead_or_behind() -> void:
	var context: AIItemBrain.ItemDecisionContext = AIItemBrain.ItemDecisionContext.new()
	var use_profile: AIItemUseProfile = AIItemUseProfile.new()
	assert_false(AIItemBrain.is_rule_eligible(ItemData.ItemCategory.PROJECTILE, use_profile, context))
...
func test_nitro_can_ineligible_while_already_boosting() -> void:
	...
	var use_profile: AIItemUseProfile = AIItemUseProfile.new()
	assert_false(AIItemBrain.is_rule_eligible(ItemData.ItemCategory.BOOST, use_profile, context))
...
func test_nitro_can_eligible_on_a_straight_when_not_boosting() -> void:
	...
	var use_profile: AIItemUseProfile = AIItemUseProfile.new()
	assert_true(AIItemBrain.is_rule_eligible(ItemData.ItemCategory.BOOST, use_profile, context))
...
func test_storm_beacon_eligible_only_from_third_place_or_worse() -> void:
	...
	leading.rank = 2
	assert_false(AIItemBrain.is_rule_eligible(ItemData.ItemCategory.LEADER_STRIKE, use_profile, leading))
	var trailing: AIItemBrain.ItemDecisionContext = AIItemBrain.ItemDecisionContext.new()
	trailing.rank = 3
	assert_true(AIItemBrain.is_rule_eligible(ItemData.ItemCategory.LEADER_STRIKE, use_profile, trailing))
...
func test_should_use_is_false_without_an_item() -> void:
	...
	var view: FakeItemSlotView = FakeItemSlotView.new(false, ItemData.ItemCategory.LEADER_STRIKE)
	var context: AIItemBrain.ItemDecisionContext = AIItemBrain.ItemDecisionContext.new()
	context.rank = 5
...
func test_should_use_waits_for_the_decision_delay_before_firing() -> void:
	...
	rng.seed = 1
	var brain: AIItemBrain = AIItemBrain.new(rng)
	var view: FakeItemSlotView = FakeItemSlotView.new(true, ItemData.ItemCategory.LEADER_STRIKE)
	var context: AIItemBrain.ItemDecisionContext = AIItemBrain.ItemDecisionContext.new()
	context.rank = 5
