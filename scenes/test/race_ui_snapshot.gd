...
func _ready() -> void:
	...
	var config: RaceConfig = _config(players, load(TRACKS[track_index]) as TrackData)
	config.intro_seconds = float(args[6]) if args.size() > 6 else 0.0
	GameState.pending_race_config = config
	_manager.configure(config, func(kart: KartController, line: RacingLine) -> InputProvider:
