...
func _drive() -> void:
	...
	var controls: Dictionary = _collect_controls(lobby)
...
func _collect_controls(lobby: Node) -> Dictionary:
	var host_button: Button = _button_with_text(lobby, "HOST")
	var join_button: Button = _button_with_text(lobby, "JOIN")
	var ready_button: Button = _button_with_text(lobby, "READY")
	var start_button: Button = _button_with_text(lobby, "START")
	var back_button: Button = _button_with_text(lobby, "BACK")
	var ip_field: LineEdit = _line_edit_with_placeholder(lobby, "Host IP or join code")
	var password_field: LineEdit = _line_edit_with_placeholder(lobby, "Password (optional)")
	var status_label: Label = lobby.get_node_or_null("Panel/VBox/Footer") as Label
	var spin_boxes: Array[Node] = _find_all(lobby, "SpinBox")
	var option_buttons: Array[Node] = _find_all(lobby, "OptionButton")
...
func _find_all(root: Node, klass: String) -> Array[Node]:
	var out: Array[Node] = []
...
func _button_with_text(root: Node, text: String) -> Button:
	for node: Node in _find_all(root, "Button"):
		if (node as Button).text.strip_edges() == text:
			return node as Button
	return null


func _line_edit_with_placeholder(root: Node, placeholder: String) -> LineEdit:
	for node: Node in _find_all(root, "LineEdit"):
